...
struct Robot 
{
	...
	void draw(float startx,float starty,float startz) // pass the starting location of the robot
	{
		...
		normalMatrix=transpose(inverse(view*model));
		worldNormalMatrix=transpose(inverse(model));
		glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);
		glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
...
void mykey(unsigned char key, int x, int y)
{
	...
	if(key=='s')
	{
		...
		{
			shadeType=1;
			cout<<"Current Shading Model is Per Frag Phong Shading!"<<endl;
		}		
		else if(shadeType==1)
		{
			shadeType=0;			
...
{ 
	...
	Robo.draw(-10,20,30); //( Initial position of the robot (0,0,30) this can be changed    
...
int main(int argc, char** argv) 
{
	...
	//glutGameModeString( "1280x768:32@60" ); //the settings for fullscreen mode
     //glutEnterGameMode();
