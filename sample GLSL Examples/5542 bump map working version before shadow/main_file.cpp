...
GLuint epos;
int shadeType=0;
float Xcam=0; float Ycam= 40; float Zcam=0;
...
void SpecialKey(int key,int x, int y)
{
	switch (key) 
	{
	case GLUT_KEY_LEFT:  Xcam = Xcam-5; break;
	case GLUT_KEY_RIGHT: Xcam = Xcam+5; break;
	case GLUT_KEY_UP:    Ycam = Ycam+5; break;
	case GLUT_KEY_DOWN:  Ycam = Ycam-5; break;
	}
...
void InitFBO() 
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, 2048,2048, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, NULL); 
	...
	glReadBuffer(GL_NONE); // not in tutorial
...
void InitPly(char** argv)
{
	...
	plyObjPos[0] = vec3(0,-2,0);
	plyObjPos[1] = vec3(-50,-8,0);
	plyObjPos[2] = vec3(50,-8,0);
	plyObjPos[3] = vec3(-50,-8,-35);
	plyObjPos[4] = vec3(-50,-8,35);
	plyObjPos[5] = vec3(50,-8,-35);
	plyObjPos[6] = vec3(50,-8,35);
...
void drawModel()
{
	for(int i=0;i<totObj-4;i++)
	{
		...
		model=scale(model,vec3(300,300,280));  //vec3(10,10,10) for hand
		normalMatrix=transpose(inverse(view*model));
...
{			
	...
	modelStack.push(model);	
	model=translate(model,vec3(-70,50,30));
	model=scale(model,vec3(25,25,25));    
	normalMatrix=transpose(inverse(view*model));	
	diffuseTerm = vec4(1,1,0.2,1);	
	glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);	
	glUniform4fv(m5, 1,&diffuseTerm[0]);
	draw_sphere(); 
...
void drawStaticSceneTest()
{
	...
	modelStack.push(model);		
	model = translate(model,vec3(0,0,-80));
	model=scale(model,vec3(200,200,2));    
	normalMatrix=transpose(inverse(view*model));
	diffuseTerm = vec4(1,0,0,1);
	glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);
	glUniform4fv(m5, 1,&diffuseTerm[0]);
	draw_cube(); 
...
{ 	
	...
	//just activate bump map texture so that its accessible from shader
	...
	Robo.draw(30,0,30); //( Initial position of the robot (0,0,30) this can be changed    
