...
//          GLSL Bump Map, Shadow Map, Glossy Reflection, Refraction //
///////////////////////////////////////////////////////////////////////
...
vec3 eyePos = vec3(0.0,120.0,380.0);
...
struct Robot 
{
	...
	void draw(float startx,float starty,float startz) // pass the starting location of the robot
	{
		...
		model=scale(model,vec3(3.5,3.5,3.5));
		normalMatrix=transpose(inverse(view*model));
		glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);
		glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
		buildRobot(); 
...
void InitFBO() 
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, 4096,4096, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, NULL); 
	...
	//FBO for Reflection
	glGenFramebuffers(1, &fboHandle[2]); 
	glBindFramebuffer(GL_FRAMEBUFFER, fboHandle[2]); 
...
void InitPly(char** argv)
{
	...
	plyObjPos = new vec3[20];
	plyObjRot = new vec3[20];
	plyObjScale = new vec3[20];
	plyRotAng = new float[20];

	plyObjPos[0] = vec3(0,1,-30);
	plyObjPos[1] = vec3(-120,10,50);
	plyObjPos[2] = vec3(-200,16,60);
	plyObjPos[3] = vec3(-200,11,120);
	plyObjPos[4] = vec3(-200,16,180);
	plyObjPos[5] = vec3(200,11,0);
	plyObjPos[6] = vec3(200,16,60);
	plyObjPos[7] = vec3(200,11,120);
	plyObjPos[8] = vec3(200,16,180);
	plyObjPos[9] = vec3(-50,48,20);
	plyObjPos[10] = vec3(-50,48,120);
	plyObjPos[11] = vec3(-50,48,220);
	plyObjPos[12] = vec3(50,48,20);
	plyObjPos[13] = vec3(50,48,120);
	plyObjPos[14] = vec3(50,48,220);


	plyObjRot[0] = vec3(1,0,0);
	plyObjRot[1] = vec3(1,0,0);
	plyObjRot[2] = vec3(1,0,0);
	plyObjRot[3] = vec3(1,0,0);
	plyObjRot[4] = vec3(1,0,0);
	plyObjRot[5] = vec3(0,1,0);
	plyObjRot[6] = vec3(0,1,0);
	plyObjRot[7] = vec3(0,1,0);
	plyObjRot[8] = vec3(0,1,0);
	plyObjRot[9] = vec3(1,0,0);	
	plyObjRot[10] = vec3(1,0,0);
	plyObjRot[11] = vec3(1,0,0);
	plyObjRot[12] = vec3(1,0,0);
	plyObjRot[13] = vec3(1,0,0);
	plyObjRot[14] = vec3(1,0,0);


	plyObjScale[0] = vec3(350,350,350);
	plyObjScale[1] = vec3(400,400,350);
	plyObjScale[2] = vec3(7,6,6);
	plyObjScale[3] = vec3(7,6,6);
	plyObjScale[4] = vec3(7,6,6);
	plyObjScale[5] = vec3(7,6,6);
	plyObjScale[6] = vec3(7,6,6);
	plyObjScale[7] = vec3(7,6,6);
	plyObjScale[8] = vec3(7,6,6);
	plyObjScale[9] = vec3(500,500,500);
	plyObjScale[10] = vec3(500,500,500);
	plyObjScale[11] = vec3(500,500,500);
	plyObjScale[12] = vec3(500,500,500);
	plyObjScale[13] = vec3(500,500,500);
	plyObjScale[14] = vec3(500,500,500);
...
void drawModel()
{
	for(int i=0;i<totObj-4;i++)
	{
		...
		model = rotate(model,plyRotAng[i],plyObjRot[i]); 
		model=scale(model,vec3(plyObjScale[i]));  //for others
...
{			
	...
	modelStack.push(model);	
	model=translate(model,vec3(-70,100,90));
	...
	modelStack.push(model);	
	model=translate(model,vec3(80,150,-70));
	model=scale(model,vec3(40,40,40));    
	normalMatrix=transpose(inverse(view*model));
...
void drawScene1Reflection()
{
	...
	modelStack.push(model);	
	model=translate(model,vec3(0,280,0));
	model=scale(model,vec3(100,100,100));    
	normalMatrix=transpose(inverse(view*model));
	diffuseTerm = vec4(0.8,0.4,0.4,1);
	glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);
	glUniform4fv(m5, 1,&diffuseTerm[0]);
	draw_sphere(); 
...
void drawScene1Actual()
{
	...
	needReflection=1;
	glUniform1i(reflection,needReflection); //reflection
...
void drawScene2Firstpass()
{
	...
	///////////////Draw base cube//////////////////	
	modelStack.push(model);	
	model=translate(model,vec3(100,-4,100));
	model=rotate(model,-90.0f,vec3(1,0,0));
	model=scale(model,vec3(1000,1000,2));    
	normalMatrix=transpose(inverse(view*model));
	diffuseTerm = vec4(1,0.87,0.67,1);
	glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);
	...
	///////////////Draw base cube//////////////////	
	modelStack.push(model);	
	model=translate(model,vec3(0,-4,100));
	model=rotate(model,-90.0f,vec3(1,0,0));
	model=scale(model,vec3(100,1000,4));    
	normalMatrix=transpose(inverse(view*model));
	diffuseTerm = vec4(1,0.27,0.47,1);
	glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);
	glUniform4fv(m5, 1,&diffuseTerm[0]);
	draw_cube(); 
...
void display() 
{
	...
	plyFlag = glGetUniformLocation(programObject, "isPly");
	ref =  glGetUniformLocation(programObject, "reflectionTexture");
	reflection = glGetUniformLocation(programObject, "needReflection");
	...
	mat4 projection = perspective(70.0f,1.0f,15.0f,5000.0f); 
	view = lookAt(vec3(-20,80,450),COI,vec3(0.0,1.0, 0.0)); 
	...
	glUniform1i(reflectTex,reflectionTex); // see through
	glUniform1i(ref,refTex); //reflection sampler
	glUniform1i(reflection,needReflection); //reflection
	...
	modelStack.push(model);
	Robo.draw(60,5,30); //( Initial position of the robot (0,0,30) this can be changed   
	modelStack.pop();
	...
	normalMatrix=transpose(inverse(view*model));// normalMatrix is inverse transpose of modelView matrix
	worldNormalMatrix=transpose(inverse(model)); // normal matrix in world space.. required for cube Env mapping
	projection = perspective(70.0f,1.0f,15.0f,5000.0f);
	...
	//Create Static Scene	
	drawScene1Refraction(); // static scene is always drawn using phong shading // This is just env without all the transparent objects
	...
	/*********************************************************************************************/
	/*********************************************************************************************/
	/*   Third Pass of rendering  for Reflection*/
	/*********************************************************************************************/
	/*********************************************************************************************/
	...
	glActiveTexture(GL_TEXTURE0+refTex);	
	glBindTexture(GL_TEXTURE_2D,refTex);
	glUniform1i(ref,refTex);	
	...
	//Create Static Scene	
	drawScene1Reflection(); // static scene is always drawn using phong shading // This is just env without all the transparent objects
	...
	glViewport(0,0,width,height); // the size of the viewport is the size of the texture 
