...
uniform mat4 prevVPMat; //this is VP matrix of last frame
...
Lights light1 = Lights(vec4(-20,150,600,0),normalize(vec4(20,150,300,1)-vec4(-20,80,300,0)),0.8); // spotDir = normalize(COI-lightLoc);
...
{  
	...
	vec2 coord = gl_FragCoord.xy / vec2(800,800) + rand(texCoordinate)/80;	
	vec4 reflectionPlaneColor = texture(reflectionMap,coord); // calculate color for texture 

	//For see through object
	vec2 coord1 = gl_FragCoord.xy / vec2(800,800);
	vec4 seeThroughColor = texture(reflectionMap,coord1); // calculate color for texture 
	...
	/*if(isReflect==1)
		gl_FragColor += Iamb + mix(reflectionPlaneColor,Idiff,0.2) + Ispec;  
	else*/
	...
	float epsilon = 0.005; // increase value to remove little artifacts
	...
	gl_FragColor = shadowFactor*gl_FragColor; //Uncomment to apply shadow in this shader
