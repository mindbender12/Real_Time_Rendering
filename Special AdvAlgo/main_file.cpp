...
vec3 eyePos = vec3(0.0,150.0,300.0);
...
void InitFBO() 
{
	...
	glDrawBuffer(GL_NONE); //Important step
	glReadBuffer(GL_NONE); //Important step
...
void InitPly(char** argv)
{
	...
	plyObjPos[0] = vec3(0,-1,-320);
	plyObjPos[1] = vec3(-200,11,0);
	...
	plyObjScale[0] = vec3(3.5,5,1.6);
	plyObjScale[1] = vec3(7,6,6);
...
void drawScene1Actual()
{
	...
	normalMatrix=transpose(inverse(view*model));
	diffuseTerm = vec4(0.4,0.4,0.4,1);
	glUniformMatrix4fv(m2, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(m4, 1, GL_FALSE, &normalMatrix[0][0]);
	glUniform4fv(m5, 1,&diffuseTerm[0]);
	draw_sphere(); 
...
void display() 
{
	...
	mat4 projection = perspective(70.0f,1.0f,15.0f,1500.0f);
	...
	//drawScene2Renderpass();

	/*glActiveTexture(GL_TEXTURE0+reflectionTex);	
	glBindTexture(GL_TEXTURE_2D,reflectionTex);
	glUniform1i(texLoc,reflectionTex);*/
